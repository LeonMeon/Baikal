...
def loader_polar(Batch_size = 64, regime = "train"):
    with h5py.File(path_to_h5, 'r') as hf:
        Data = hf[regime + '/data/'][:,:32]
        Polar=hf[regime + '/ev_chars'][:,0] * (np.pi) / 180
...
def loader_vec(Batch_size = 64, regime = "train"):
    assert regime in ["train", "val", "test"]
    with h5py.File(path_to_h5, 'r') as hf:
        Data = hf[regime + '/data/'][:,:32]
        Polar = hf[regime + '/ev_chars'][:,0] * (np.pi) / 180
        Azimut = hf[regime + '/ev_chars'][:,1] * (np.pi) / 180
...
def loader_E(Batch_size = 64, regime = "train"):
    assert regime in ["train", "val", "test"]
    with h5py.File(path_to_h5, 'r') as hf:
            Data = hf[regime + '/data/'][:,:32]
            #lnE = np.log(hf[regime + "ev_chars"][i * int(tr_set_len) : (i + di) * int(tr_set_len),2] + 1)
            lnE = np.log(hf[regime + "/ev_chars"][:,2])
